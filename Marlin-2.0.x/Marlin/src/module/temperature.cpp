...
unsigned int ecm_edm_voltage = 0;
...
void Temperature::isr() {
  ...
  switch (adc_sensor_state) {
	...
	#if ENABLED(ECM_EDM_MODE)
      case Prepare_VOLTAGE_DISTANCE: HAL_START_ADC(ECM_EDM_PIN); break;
      case Measure_VOLTAGE_DISTANCE:
        if (!HAL_ADC_READY()) next_sensor_state = adc_sensor_state; // Redo this state
        else ecm_edm_voltage = HAL_READ_ADC();
      break;
    #endif
