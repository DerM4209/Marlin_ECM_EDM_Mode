...
if (ecm_edm_voltage < TARGET_VOLTAGE){
planner.quick_stop();
planner.synchronize();
reverse = true;
...
if (new_destination != target_position && new_destination != origin_position){ //if entered position is different from target position and not the origin position
